...
list_year_average_defen = [0]*11
for index,year in enumerate(list_year):
    Return_defen = [0]*1000
    for i in range(1000):
        defen_investor = D_Investor(12000, "20%s-01-03"%list_year[index])
        Return_defen[i] = Invest(defen_investor,"20%s-12-31"%list_year[index])
    list_year_average_defen[index] = math.floor(sum(Return_defen)/1000)
print(list_year_average_defen)

list_year_average_aggre = [0]*11
for index,year in enumerate(list_year):
    Return_aggre = [0]*1000
    for i in range(1000):
        aggr_investor = A_Investor(12000,"20%s-01-03"%list_year[index])
        Return_aggre[i] = Invest(aggr_investor,"20%s-12-31"%list_year[index])
    list_year_average_aggre[index] = math.floor(sum(Return_aggre)/1000)
print(list_year_average_aggre)

list_year_average_mix = [0]*11
for index,year in enumerate(list_year):
    Return_mix = [0]*1000
    for i in range(1000):
        mix_investor = M_Investor(12000,"20%s-01-03"%list_year[index])
        Return_mix[i] = Invest(mix_investor,"20%s-01-03"%list_year[index])
    list_year_average_mix[index] = math.floor(sum(Return_mix)/1000)
print(list_year_average_mix)
